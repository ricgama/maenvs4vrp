...
BENCHMARK_INSTANCES_PATH = 'mdvrptw/data/benchmark'

class BenchmarkInstanceGenerator(InstanceBuilder):
    """
    MDVRPTW benchmark instances generation class.
    """
    @classmethod
    ...
    def __init__(self, 
        ...
        assert instance_type in ["Vidal"], f"instance unknown type"
    ...
    def parse_instance_data(self, instance_data: list, instance_name:str) -> Dict:
        ...
        for data in instance_data[(instance['num_nodes']+1):]:
            coords.append([float(data[1]), float(data[2])])
            demands.append(float(data[4]))
            time_windows.append([float(data[-2]), float(data[-1])])
            service_time.append(float(data[3]))
        for data in instance_data[(instance['num_depots']+1):instance['num_nodes']+1]:
            coords.append([float(data[1]), float(data[2])])
            demands.append(float(data[4]))
            time_windows.append([float(data[-2]), float(data[-1])])
            service_time.append(float(data[3]))
        ...
        capacity = float(instance_data[1][1])

        self.depot_idx = torch.arange( 0, instance['num_depots'], device=self.device).repeat((*self.batch_size, 1))
        data['depot_idx'] = self.depot_idx
        ...
        if self.instance_type in ['Vidal']:
    ...
    def sample_first_n_services(self, 
        ...
        new_instance['num_depots'] = instance['num_depots']
    ...
    def random_sample_instance(self, 
        ...
        index = torch.cat([idxs[data['is_depot']], idxs[~data['is_depot']][torch.randperm(instance['num_nodes']-instance['num_depots'])]], dim=0)[:new_instance['num_nodes']]
...
if __name__ == '__main__':
    ...
    generator = BenchmarkInstanceGenerator(instance_type='Vidal', set_of_instances={'pr11a'})
