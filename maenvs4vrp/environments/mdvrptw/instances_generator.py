...
if __name__ == '__main__':
    ...
    for num_nodes, n_agent, num_depots in [(105, 5, 5), (55, 5, 5)]:
        ...
        for k in range(number_instances):
            ...
            name = f'generated_val_servs_{num_nodes-1}_agents_{n_agent}_depots_{num_depots}_{k}'
            instance['name'] = name
            if not os.path.exists(f'data/generated/servs_{num_nodes-num_depots}_agents_{n_agent}_depots_{num_depots}/validation'):
                os.makedirs(f'data/generated/servs_{num_nodes-num_depots}_agents_{n_agent}_depots_{num_depots}/validation')
            with open(f'data/generated/servs_{num_nodes-num_depots}_agents_{n_agent}_depots_{num_depots}/validation/'+name+'.pkl', 'wb') as fp:
                pickle.dump(instance, fp, protocol=pickle.HIGHEST_PROTOCOL)

            instance =  generator.sample_instance(num_agents=n_agent, num_nodes=num_nodes, num_depots=num_depots)
            name = f'generated_test_servs_{num_nodes-1}_agents_{n_agent}_depots_{num_depots}_{k}'
            instance['name'] = name
            if not os.path.exists(f'data/generated/servs_{num_nodes-num_depots}_agents_{n_agent}_depots_{num_depots}/test'):
                os.makedirs(f'data/generated/servs_{num_nodes-num_depots}_agents_{n_agent}_depots_{num_depots}/test')
            with open(f'data/generated/servs_{num_nodes-num_depots}_agents_{n_agent}_depots_{num_depots}/test/'+name+'.pkl', 'wb') as fp:
                pickle.dump(instance, fp, protocol=pickle.HIGHEST_PROTOCOL)
