...
class Environment(AECEnv):
    """
    PDPTW environment generator class.
    """
    def __init__(self,
    ...
    def reset(self, 
        ...
        self.td_state['cur_agent'] = TensorDict({
                                ...
                                'pending_deliveries': self.td_state['agents']['pending_deliveries'].gather(1, self.td_state['cur_agent_idx'][:,:,None].expand(-1, -1, self.num_nodes)).squeeze(1),
    ...
    def _update_state(self, action):
        ...
        self.td_state['cur_agent','pending_deliveries'].scatter_(1, action, False)
        self.td_state['agents','pending_deliveries'].scatter_(1, 
                                            self.td_state['cur_agent_idx'][:,:,None].expand(-1,-1,self.num_nodes), self.td_state['cur_agent','pending_deliveries'].unsqueeze(1))
        ...
        # is agent is done set pending_deliveries to False in active_nodes_mask
        pending_mask = self.td_state['nodes', 'active_nodes_mask']  * ~self.td_state['cur_agent','pending_deliveries']
        self.td_state['nodes', 'active_nodes_mask'] = torch.where(agents_done, pending_mask, 
                                                             self.td_state['nodes', 'active_nodes_mask'])
