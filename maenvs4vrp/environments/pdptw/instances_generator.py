...
GENERATED_INSTANCES_PATH = 'pdptw/data/generated'

class InstanceGenerator(InstanceBuilder):
    """
    PDPTW instances generation class.
    """
    @classmethod
    ...
    def get_time_windows(self, instance:TensorDict=None, 
                         pickup_idx:TensorDict=None, 
                         delivery_idx :TensorDict=None, 
        ...
        pickup_coords = instance['coords'].gather(1, pickup_idx[:, :, None].expand(-1, -1, 2))
        ...
        num_services = (self.max_num_nodes-1)//2
    ...
    def random_generate_instance(self, num_agents:int=20, 
        ...
        self.depot_idx = 0
        instance['depot_idx'] = self.depot_idx * torch.ones((*self.batch_size, 1), dtype = torch.long, device=self.device)
        ...
        delivery_idx = torch.arange(num_services+1, self.max_num_nodes, device=self.device).repeat(self.batch_size.numel(),1)

        instance['pickup_idx'] = torch.zeros((*self.batch_size, self.max_num_nodes), dtype = torch.long, device=self.device)
        instance['delivery_idx'] = torch.zeros((*self.batch_size, self.max_num_nodes), dtype = torch.long, device=self.device)

        instance['pickup_idx'].scatter_(1, delivery_idx, pickup_idx) #For pickup tasks, the PICKUP index is 0, whereas the DELIVERY sibling gives the index of the corresponding delivery task
        instance['delivery_idx'].scatter_(1, pickup_idx, delivery_idx)  # For delivery tasks, the PICKUP index gives the index of the corresponding pickup task. 

        instance['is_pickup'] = torch.zeros((*self.batch_size, self.max_num_nodes), dtype=torch.bool, device=self.device)
        instance['is_pickup'].scatter_(1, pickup_idx, True)

        instance['is_delivery'] = torch.zeros((*self.batch_size, self.max_num_nodes), dtype=torch.bool, device=self.device)
        instance['is_delivery'].scatter_(1, delivery_idx, True)

        demands = torch.randint(low = 1, high=11, size = (*self.batch_size, num_services), dtype = torch.float, device=self.device)
        instance['demands'] = torch.zeros((*self.batch_size, self.max_num_nodes), dtype = torch.float, device=self.device)
        instance['demands'].scatter_(1, pickup_idx, demands)
        instance['demands'].scatter_(1, delivery_idx, -demands)

        service_times = self.service_times * torch.ones((*self.batch_size, self.max_num_nodes), dtype = torch.float, device=self.device)
        service_times[:, self.depot_idx] = 0
        instance['service_time'] = service_times

        time_windows = self.get_time_windows(instance, pickup_idx, delivery_idx, self.batch_size, seed)

        instance['tw_low'] =  time_windows[:, :, 0].clone()
        instance['tw_high'] = time_windows[:, :, 1].clone()

        instance['is_depot'] = torch.zeros((*self.batch_size, self.max_num_nodes), dtype=torch.bool, device=self.device)
